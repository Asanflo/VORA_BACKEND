...
class AIVoiceService:
    ...
    async def parse_voice_text(
        ...
        Utilise l'API Gemini si disponible, ou le moteur local expert en cas de fonctionnement hors-ligne.
        """
        clean_text = raw_text.strip()
        parsed_data = None

        # 1. Tentative avec Gemini API si clé configurée
        if settings.GEMINI_API_KEY:
            try:
                parsed_data = await cls._call_gemini_api(clean_text)
        ...
        if pickup_text:
            ...
            # Si le départ n'est pas dicté, utiliser la position GPS actuelle
            nearest = await POIService.find_nearest_poi(session, user_current_lat, user_current_lng)
            if nearest:
                matched_pickup_poi = nearest.poi

        # Si pas de départ défini, prendre un repère central par défaut (ex: Poste Centrale Yaoundé)
    ...
    @classmethod
    async def _call_gemini_api(cls, text: str) -> Optional[Dict[str, Any]]:
        ...
        payload = {
            "contents": [
                {
                    "parts": [
                        {"text": f"{CAMEROON_NLU_SYSTEM_PROMPT}\n\nPhrase utilisateur : \"{text}\""}
                    ]
                }
            ]
        }
    ...
    def _local_cameroon_nlu_parser(cls, text: str) -> Dict[str, Any]:
        ...
        # Modèle 1 : Départ et Arrivée ("Je quitte X pour Y" / "De X à Y" / "From X go Y" / "I dey for X carry me go Y")
        ...
        # Modèle 2 : Destination directe en Pidgin ("drop me for X", "carry me go X", "i wan go X")
        ...
        # Modèle 3 : Destination directe en Camfranglais / Français ("tu me drop à X", "je vais à X", "laisse-moi au X", "direction X")
        if not dropoff_landmark:
        ...
        # Nettoyage des mots parasites en fin de chaîne
        for _ in range(3):
            if dropoff_landmark:
        ...
        # Si toujours rien, prendre le texte complet comme repère potentiel
        if not dropoff_landmark:
